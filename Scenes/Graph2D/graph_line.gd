...
func Redraw(new_scaling:Vector2) -> void:
	for i in range(get_point_count()):
		prints("line move point from ", get_point_position(i), "to", get_point_position(i) * new_scaling)
		set_point_position(i, get_point_position(i) * new_scaling)
