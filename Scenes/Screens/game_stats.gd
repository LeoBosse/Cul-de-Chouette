...
signal undoing_turn(point_correction:Array, current_player:int, civets:Array, grelottines:Array)
...
func Setup(player_names_list:Array):
	...
	grelottine_history.resize(nb_players)
	for i in range(nb_players):
		player_scores[i] = [0]
		civet_history[i] = [false]
		grelottine_history[i] = [false]
...
func RegisterNewState(players:Array, current_player:int):
	...
	for i in range(nb_players):
		...
		grelottine_history[i].append(players[i].has_grelottine)
...
func GetExport() -> Dictionary:
	return {"names":player_names,
			...
			"grelottine_history":grelottine_history}
...
func Import(data:Dictionary):
	...
	for i in len(data["scores"][0]):
		for p in range(nb_players):
			...
			new_scores[p].has_grelottine = data["grelottine_history"][p][i]
...
func InitGraph():
	...
	%GraphScore.Initialize(line_points, player_names, 0, 1, 0, 400)
